module Docs.Tick.Example1 exposing (main)
...
customTick number =
  let
    color =
      ...
      else if number < 80 then Colors.green
      else Colors.pinkLight

    label = Junk.label color (toString number)
    even = number % 20 == 0
  in
  Tick.custom
    ...
    , width = 2
    , length = 2
    , grid = not even -- Add or remove grid!
    , direction = if even then Tick.positive else Tick.negative
    , label = Just label
    }
